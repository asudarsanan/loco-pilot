...
fn test_binary_executes() {
    ...
    assert!(
        ...
        "loco-pilot should execute successfully"
    );
}
...
fn test_config_display() {
    ...
    assert!(
        output.status.success(),
        "Config command should execute successfully"
    );

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Current configuration:"),
        "Config output should display current configuration"
    );
}
...
fn test_style_options() {
    ...
    assert!(
        minimal_output.status.success(),
        "Minimal style should execute successfully"
    );

    let minimal_stdout = String::from_utf8_lossy(&minimal_output.stdout);
    assert_eq!(
        minimal_stdout, "$ ",
        "Minimal style should be a simple dollar sign and space"
    );
    ...
    assert!(
        ...
        "Info style should execute successfully"
    );

    let info_stdout = String::from_utf8_lossy(&info_output.stdout);
    assert!(
        info_stdout.contains("["),
        "Info style should contain time in square brackets"
    );
    assert!(
        info_stdout.contains("@"),
        "Info style should contain username@hostname format"
    );
}
