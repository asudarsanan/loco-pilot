...
impl FilesystemMock {
    ...
    fn create_mock_git_repo(&self) -> PathBuf {
        ...
        writeln!(file, "0123456789abcdef0123456789abcdef01234567").expect("Failed to write to branch file");
...
"#;
        ...
        file.write_all(config_content.as_bytes()).expect("Failed to write config content");
...
fn test_with_mock_filesystem() {
    ...
    assert!(mock_git_repo.join(".git").exists(), "Mock git repo should have .git directory");
